...
namespace Infrastructure.Common
{
    ...
    public static class PropertyCopy
    {
        ...
        public static void Copy<TSource, TTarget>(TSource source, TTarget target, List<string> excludeProperties = null)
            where TSource : class
            where TTarget : class
        {
            PropertyCopier<TSource, TTarget>.Copy(source, target, excludeProperties);
        }
    }
    ...
    internal static class PropertyCopier<TSource, TTarget>
    {
        ...
        internal static void Copy(TSource source, TTarget target, List<string> excludeProperties = null)
        {
            if (initializationException != null)
            {
                throw initializationException;
            ...
            {
                if (excludeProperties != null && excludeProperties.IndexOf(sourceProperties[i].Name) > -1)
                {
                    continue;
                }
                //if (!IsSimple(sourceProperties[i].PropertyType))
                //{
                //    continue;
                //}
