$(document).ready(function () {
    var connection = new signalR.HubConnectionBuilder().withUrl("/chatDetailHub").build();
    ...
    connection.on("addUser", function (user) {
        viewModel.userAdded(new ChatUser(user.email, user.fullName, user.avatar, user.currentRoom, user.device));
    });

    connection.on("removeUser", function (user) {
        viewModel.userRemoved(user.email);
    });
    ...
    function ChatUser(email, displayName, avatar, currentRoom, device) {
        var self = this;
