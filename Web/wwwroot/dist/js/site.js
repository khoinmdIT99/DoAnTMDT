...
function openInPopUpEdit(id) {
    showInPopup('/Administrator/Slider/CreateSlider/' + id);
}

function showPopUpDelete(id) {
    //document.getElementById("btnDelete").setAttribute("onclick", "`('/Administrator/Slider/DeleteSlider/" + id + "')");
    $('#delete-modal #Slider_Id').val(id);
    $('#delete-modal').modal('show');
}

function deleteSlider() {
    
    var id = $('#delete-modal #Slider_Id').val();
    alert(id);
    $.ajax({
            type: 'POST',
            url: "/Administrator/Slider/DeleteSlider/" + id,
            success: function (res) {
                if (res.isDeleted) {
                    ...
                    $('#delete-modal').modal('hide');
                    toastr.info("Slider not deleted");
                }
            },
            error: function (err) {
                $('#delete-modal').modal('hide');
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.isValid) {
                    ...
                    toastr.success("Slider submitted successfuly", "Submit slider");
                }
                else {
